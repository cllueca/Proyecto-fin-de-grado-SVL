<template>
    <div class="container mt-5">
        <div class="row d-flex justify-content-center align-items-center" id="divContenedorForm">
            <div class="col-md-8">
                <form id="regForm">
                    <h1 id="register">Iniciar sesi&oacute;n</h1>

                    <div v-if="this.$store.state.messageLogin == null" id="errorVoz" class="hide"></div>
                    <div v-else id="errorVoz">{{ this.$store.state.messageLogin }}</div>
                    ...
                    <div class="tab" style="display: none;">
                        <h5>
                            Demuéstralo con tu <span id="opcionPwd">voz</span>:
                        </h5>
...
export default {
    ...
    methods: {
        ...
        cambiarTipoPwd() {
            ...
            if(textoPwd.textContent == 'voz')
            {
                textoPwd.textContent="contraseña";
            }
            else
            {
                textoPwd.textContent="voz";
            }
            ...
            if (this.existePwd())
            {
                ...
                if (this.voice_password != null)
                {
                    this.getBlobBase64Data(this.voice_password).then((base64Data) => {
                        ...
                        try {
                            store.dispatch("doLogin", jsonData);
                        } catch (error) {
                            console.error(error);
                        }
                    });
                ...
                else // Contraseña normal
                {
                    ...
                    try {
                        await store.dispatch("doLogin", jsonData);
                    } catch (error) {
                        console.error(error);
                    }
                }
